...
def lr_scheduler(base_value, final_value, iters, warmup_steps=0,
                     start_warmup_value=0, lr_type='cos'):
    ...
    its = np.arange(iters - warmup_iters)
    # schedule = np.array([final_value + 0.5 * (base_value - final_value) * (1 + math.cos(math.pi * i / (len(iters)))) for i in iters])
    # schedule = np.concatenate((warmup_schedule, schedule))
    # print(schedule)
    if lr_type == 'cos':
        schedule = np.array(its)
        schedule = final_value + 0.5 * (base_value - final_value) * (1 + np.cos(math.pi * its / (len(its))))
        schedule = np.concatenate((warmup_schedule, schedule))
    else:
        schedule = base_value * np.ones((iters - warmup_iters))
        schedule = np.concatenate((warmup_schedule, schedule))
    # print(schedule)
