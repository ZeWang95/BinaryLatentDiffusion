...
class CachedImageFolder(DatasetFolder):
    ...
    def __init__(self, root, ann_file='', img_prefix='', transform=None, neg_tranform=None, target_transform=None,
        ...
        self.neg_transform = neg_tranform
    def __getitem__(self, index):
        """
        Args:
            index (int): Index
        ...
        negative = self.neg_transform(image)[0]
        
        # image = self.transform.posts(image)
        # negative = self.transform.posts(negative)
        # negative = self.neg_transform(image)[0]

        return image, negative, target

class CachedImageFolder_ori(DatasetFolder):
    """A generic data loader where the images are arranged in this way: ::
        root/dog/xxx.png
        root/dog/xxy.png
        root/dog/xxz.png
    ...
    def __init__(self, root, ann_file='', img_prefix='', transform=None, target_transform=None,
                 loader=default_img_loader, cache_mode="no"):
        super(CachedImageFolder_ori, self).__init__(root, loader, IMG_EXTENSIONS,
                                                ann_file=ann_file, img_prefix=img_prefix,
                                                transform=transform, target_transform=target_transform,
                                                cache_mode=cache_mode)
        self.imgs = self.samples
